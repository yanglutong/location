...
import com.sm.android.locations.locations.Activity.AddParam.Adapter.TestBaseAdapterWhite;
 ...
 * 参数列表
 */
public class ParamWhiteActivity extends BaseActivity implements View.OnClickListener {
    ImageView iv_findish;
    private RecyclerView recyclerView;
    private List<TestData> data = new ArrayList<>();
    private TestBaseAdapterWhite mAdapter;
    private DBManagerAddParamWhite dbManagerAddParam;
    List<AddPararBeanWhite> dataAll = null;

    TextView nullData;
    TestBaseAdapterWhite.WhiteITTestBaseAdapterCallBack itTestBaseAdapterCallBack = new TestBaseAdapterWhite.WhiteITTestBaseAdapterCallBack() {
        @Override
        public void Call(int id, boolean checkFlag, CheckBox checkBox) {
            Log.d("opjiohiho", "Call: " + id + "boolean---" + checkFlag);
            try {
                AddPararBeanWhite forid = dbManagerAddParam.forid(id);
                forid.setCheckbox(checkFlag);
                int i1 = dbManagerAddParam.updateCheck(forid);
                Log.d("nzqqqqqqqq", "Call: "+i1);

                List<AddPararBeanWhite> dataAll = dbManagerAddParam.getDataAll();
                List<AddPararBeanWhite> list = new ArrayList<>();
...
//            setadapter();
            ...
            GridLayoutManager mLayoutManager = new GridLayoutManager(ParamWhiteActivity.this, 1);
            recyclerView.setLayoutManager(mLayoutManager);
//            recyclerView.addItemDecoration(new GridSpacingItemDecoration(1, 12, false));
            mAdapter = new TestBaseAdapterWhite(ParamWhiteActivity.this, itTestBaseAdapterCallBack);
    ...
    protected void findViews() {
        ...
        UtilsView.setViewVisibility(this, layout, title, imageView, "非控IMSI", false, iv_findish, true);
...
//                        Toast.makeText(getApplicationContext(), message + itemData.phone, Toast.LENGTH_SHORT).show();
    ...
    private void setadapter() {
        recyclerView = (RecyclerView) findViewById(R.id.recyclerView);
        try {
            dbManagerAddParam = new DBManagerAddParamWhite(ParamWhiteActivity.this);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        try {
...
//        recyclerView.addItemDecoration(new GridSpacingItemDecoration(1, 12, false));
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.iv_add:
                startActivity(new Intent(ParamWhiteActivity.this, AddParamWhiteActivity.class));
                break;
        }
    }
