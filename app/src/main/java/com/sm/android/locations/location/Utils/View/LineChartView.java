package com.sm.android.locations.location.Utils.View;
...
public class LineChartView extends View {
    ...
    private int lineColor = Color.parseColor("#10A146");//曲线颜色
    private float lineWidthDP = 2f;//曲线宽度dp

    private int pointColor = Color.parseColor("#10A146");//锚点颜色
    ...
    private int pointTextColor = Color.parseColor("#6488F4");//锚点文本颜色
    private float pointTextSizeSP = 10f;//锚点文本大小


    private int lineColor2 = Color.parseColor("#FF6367");//曲线颜色
//    private float lineWidthDP = 2f;//曲线宽度dp

    private int pointColor2 = Color.parseColor("#FF6367");//锚点颜色
//    private float pointWidthDefault = 8f;
//    private float pointWidthDP = pointWidthDefault;//锚点宽度dp

//    private int tableColor2 = Color.parseColor("#BBBBBB");//表格线颜色
////    private float tableWidthDP = 0.5f;//表格线宽度dp
//
//    private int rulerTextColor2 = tableColor;//表格标尺文本颜色
////    private float rulerTextSizeSP = 10f;//表格标尺文本大小
//
//    private int pointTextColor2 = Color.parseColor("#6488F4");//锚点文本颜色
//    private float pointTextSizeSP = 10f;//锚点文本大小
    ...
    private void setupView() {
        ...
        pointPaint2.setColor(pointColor2);
...
//
        textPointPaint2 = new Paint();
        textPointPaint2.setAntiAlias(true);
        textPointPaint2.setStyle(Paint.Style.FILL);
        textPointPaint2.setTextAlign(Paint.Align.CENTER);
        textPointPaint2.setColor(pointColor2);//文本颜色
        textPointPaint2.setTextSize(sp2px(pointTextSizeSP));//字体大小
        linePath = new Path();//频点1
//        linePath2 = new Path();//频点2
        tablePath = new Path();
...
//        drawLinePoints2(canvas);//绘制曲线上的点
     ...
     * @param y
     */
    private void drawLinePointText2(Canvas canvas, String text, float x, float y) {
        textPointPaint.setTextAlign(Paint.Align.CENTER);
        float newY = y - rulerValuePadding;
        drawText(canvas, textPointPaint2, text, x, newY);
    }
    ...
    private void drawTable(Canvas canvas) {
        ...
        do {
            ...
            //绘制y轴刻度单位
            Log.d(TAG, "drawTable: " + String.valueOf(startValue));
            drawRulerYText(canvas, String.valueOf(startValue), stepStart, startHeight);//  Y  轴
...
//        drawRulerXValue2(canvas);
     ...
     * @param canvas
     */
//    private void drawRulerXValue2(Canvas canvas) {
//        if (linePoints2 == null) return;
//        for (int i = 0; i < linePoints2.length; i++) {
//            Point point = linePoints[i];
//            if (point == null) break;
//            drawRulerXText(canvas, String.valueOf(i), linePoints2[i].x, 0);
//        }
//    }
    /**
     * 绘制曲线
    ...
    private void drawLine2(Canvas canvas) {
        if (isPlayAnim) {
            Path dst = new Path();
            PathMeasure measure = new PathMeasure(linePath2, false);
            measure.getSegment(0, currentValue * measure.getLength(), dst, true);
            canvas.drawPath(dst, linePaint2);
        } else {
     ...
     * @param canvas
     */
    private void drawLinePoints2(Canvas canvas) {
        if (linePoints == null) return;

        float pointWidth = dip2px(pointWidthDP) / 2;
        int pointCount = linePoints2.length;
        if (isPlayAnim) {
            pointCount = Math.round(currentValue * linePoints2.length);
        }
        for (int i = 0; i < pointCount; i++) {
            Point point = linePoints2[i];
            if (point == null) break;
            if (isCubePoint) {
                canvas.drawPoint(point.x, point.y, pointPaint2);
            } else {
                canvas.drawCircle(point.x, point.y, pointWidth, pointPaint2);
            }
            //绘制点的文本
            drawLinePointText2(canvas, String.valueOf(dataList2.get(i).getValue()), point.x, point.y);
        }
    }
    ...
    private void setupLine2() {
        if (dataList2.isEmpty()) return;

        int stepTemp = getTableStart();
        Point pre = new Point();
        pre.set(stepTemp, -getValueHeight(dataList2.get(0).getValue()));//坐标系从0,0默认在第四象限绘制
        linePoints2[0] = pre;
        linePath2.moveTo(pre.x, pre.y);
        ...
        for (int i = 1; i < dataList2.size(); i++) {
            ...
            if (isBezierLine) {
                ...
                linePath2.cubicTo(p1.x, p1.y, p2.x, p2.y, next.x, next.y);//创建三阶贝塞尔曲线
            } else {
                linePath2.lineTo(next.x, next.y);
            }
     ...
     * @param dataList
     */
    public void setData(List<Data> dataList, List<Data> dataList2) {
        if (dataList == null) {
        ...
        //
        if (dataList2 == null) {
            throw new RuntimeException("dataList2 cannot is null!");
        }
        if (dataList2.isEmpty()) return;
        this.dataList2.clear();

        this.dataList2.addAll(dataList2);


        List<Data> dataListMy = new ArrayList<>();
//        dataListMy.add(new Data(0));
//        dataListMy.add(new Data(20));
//        dataListMy.add(new Data(40));
//        dataListMy.add(new Data(60));
//        dataListMy.add(new Data(80));
//        dataListMy.add(new Data(100));
//        dataListMy.add(new Data(120));
//        dataListMy.add(new Data(140));
//        dataListMy.add(new Data(160));
//        dataListMy.add(new Data(180));
        dataListMy.add(new Data(0));
//        dataListMy.add(new Data(10));
//        dataListMy.add(new Data(20));
//        dataListMy.add(new Data(30));
//        dataListMy.add(new Data(40));
//        dataListMy.add(new Data(50));
//        dataListMy.add(new Data(60));
//        dataListMy.add(new Data(70));
//        dataListMy.add(new Data(80));
//        dataListMy.add(new Data(90));
        dataListMy.add(new Data(1100));
//        dataListMy.add(new Data(110));
        maxValue = Collections.max(dataListMy, new Comparator<Data>() {
        ...
        minValue = Collections.min(dataListMy, new Comparator<Data>() {
            @Override
            public int compare(Data o1, Data o2) {
                return o1.getValue() - o2.getValue();
            }
